...
describe("searchDocumentsInMemory", () => {
  it("matches title/citation/snippet and clamps limit", () => {
    ...
    expect(result.documents[0]?.id).toBe("a-1");
  });
});

describe("createRegexCitationHandlers", () => {
  it("parses and validates expected format", async () => {
    ...
    expect(parsed?.normalized).toBe("XX-2024-15");
    ...
    expect(invalid.reason).toBe("Bad format");
  });
});
